...
class SalesAchievementCustomerEmployee(models.Model):
    _name = 'sales.achievement.customer.employee'
    _auto = False
    ...
    def init(self, check_date = '', check_date_gte_or_lte = '', date_gte = datetime.datetime.now().strftime('%Y/%m/%d'), date_lte = datetime.datetime.now().strftime('%Y/%m/%d')):
        ...
        self._cr.execute("""
        CREATE OR REPLACE VIEW sales_achievement_customer_employee AS

        SELECT 
            row_number() OVER(ORDER BY hr_employee_employee_code, res_partner_customer_code) AS id , 
...
--                        ON account_move_line.partner_id = res_employee.join_partner_id
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        try:
            print_all_button = request.session['print_all_button_sales_achievement_customer_employee']
        except:
            request.session['print_all_button_sales_achievement_customer_employee'] = False
        ...
        if ctx.get('have_advance_search') or print_all_button:
            ...
            return super(SalesAchievementCustomerEmployee, self).search(domain, offset=offset, limit=limit, order=order, count=count)
        return []
    ...
    def _get_condition_search_of_module(self, args):
        ...
        args_init = {'date_gte': '',
                     'date_lte': ''}
        # sales_achievement_customer_employee_context = self._context.copy()

        # Save advanced_search_arguments to session
        request.session['advanced_search_arguments_of_customer_employee'] = args
        ...
        # ===========================================
        request.session['advanced_search_condition_of_customer_employee'] = {uid: dict_domain_in_search}
    ...
    def print_all_sales_achievement_customer_employee(self, args, offset=0, limit=None, order=None, count=False):

        # ==============================================
        # Set session flag to True if from Print Button
        # ==============================================
        request.session['print_all_button_sales_achievement_customer_employee'] = True

        # ==============================================
        # Get advanced_search arguments from session
        # ==============================================
        args = request.session['advanced_search_arguments_of_customer_employee']
        ...
        if len(sales_info_ids) > 0:
            ...
            # ==============================================
            return self.env.ref('Maintain_Achievement_Management.report_sales_achievement_customer_employee')\
