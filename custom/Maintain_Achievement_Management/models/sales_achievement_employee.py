...
# dict_domain_employee = {}


class SalesAchievementEmployee(models.Model):
    _name = 'sales.achievement.employee'
    _auto = False
    ...
    def init(self, check_date = '', check_date_gte_or_lte = '', date_gte = datetime.datetime.now().strftime('%Y/%m/%d'), date_lte = datetime.datetime.now().strftime('%Y/%m/%d')):
        # stringvo(self, check_date, date_gte, date_lte)
        tools.drop_view_if_exists(self._cr, self._table)
        self._cr.execute("""
        CREATE OR REPLACE VIEW sales_achievement_employee AS

        SELECT 
            row_number() OVER(ORDER BY hr_employee_employee_code) AS id ,
...
--        SELECT 
--            HR.employee_code AS hr_employee_employee_code,
--            HR.name AS hr_employee_name,
--
--            DETAIL.sum_pay_amount_final_no_tax AS sum_pay_amount_final_no_tax,
--            DETAIL.sum_pay_amount_final_include_tax AS sum_pay_amount_final_include_tax,
--
--            DETAIL.sum_return_amount_final_no_tax AS sum_return_amount_final_no_tax,
--            DETAIL.sum_return_amount_final_include_tax AS sum_return_amount_final_include_tax,
--
--            DETAIL.sum_discount_amount_final_no_tax AS sum_discount_amount_final_no_tax,
--            DETAIL.sum_discount_amount_final_include_tax AS sum_discount_amount_final_include_tax,
--
--            DETAIL.sum_cost_price_amount_final_no_tax AS sum_cost_price_amount_final_no_tax,
--            DETAIL.sum_cost_price_amount_final_include_tax AS sum_cost_price_amount_final_include_tax,
--
--            DETAIL.net_sale_amount_final_no_tax AS net_sale_amount_final_no_tax,
--            DETAIL.net_sale_amount_final_include_tax AS net_sale_amount_final_include_tax,
--
--            DETAIL.net_sale_amount_final_no_tax AS gross_amount_final_no_tax,
--            DETAIL.net_sale_amount_final_include_tax AS gross_amount_final_include_tax
--
...
--                GROUP BY date, hr_employee_employee_code, hr_employee_name
--                ORDER BY hr_employee_employee_code, date
--                ) AS employee_option_no_search
            --WHERE hr_employee_employee_code IS NOT NULL
            GROUP BY hr_employee_employee_code, hr_employee_name
            )
--            ) AS DETAIl
--        ON HR.employee_code = DETAIl.hr_employee_employee_code
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        try:
            print_all_button = request.session['print_all_button_sales_achievement_employee']
        except:
            request.session['print_all_button_sales_achievement_employee'] = False
        ...
        if ctx.get('have_advance_search') or print_all_button:
            ...
            return super(SalesAchievementEmployee, self).search(domain, offset=offset, limit=limit, order=order, count=count)
        return []
    ...
    def _get_condition_search_of_module(self, args):
        ...
        args_init = {'date_gte': '',
                     'date_lte': ''}
        # sales_achievement_employee_context = self._context.copy()

        # Save advanced_search_arguments to session
        request.session['advanced_search_arguments_of_employee'] = args
        ...
        # if sales_achievement_employee_context and 'sales_achievement_employee' in sales_achievement_employee_context:
        for record in args:
            if record[0] == '&':
                continue
        ...
        # dict_domain_employee = {uid: dict_domain_in_search}

        # ===========================================
        # Save advanced_search domain to session
        # ===========================================
        request.session['advanced_search_condition_of_employee'] = {uid: dict_domain_in_search}
    ...
    def passConditionInSearchToReport(self):
        ...
        # ===========================================
        advanced_search_domain_employee = request.session['advanced_search_condition_of_employee']
        list_domain = [advanced_search_domain_employee[uid]]

        return list_domain

    def print_all_sales_achievement_employee(self, args, offset=0, limit=None, order=None, count=False):

        # ==============================================
        # Set session flag to True if from Print Button
        # ==============================================
        request.session['print_all_button_sales_achievement_employee'] = True

        # ==============================================
        # Get advanced_search arguments from session
        # ==============================================
        args = request.session['advanced_search_arguments_of_employee']
        ...
        if len(sales_info_ids) > 0:
            ...
            # ==============================================
            return self.env.ref('Maintain_Achievement_Management.report_sales_achievement_employee')\
