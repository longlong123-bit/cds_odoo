...
class SalesAchievementEmployee(models.Model):
    ...
    sum_pay_amount_final_include_tax = fields.Float(string='Sum Pay Amount Include Tax', readonly=True)
    sum_return_amount_final_include_tax = fields.Float(string='Sum Return Amount Include Tax', readonly=True)
    sum_discount_amount_final_include_tax = fields.Float(string='Sum Discount Amount Include Tax', readonly=True)
    sum_cost_price_amount_final_include_tax = fields.Float(string='Sum Cost Price Amount Include Tax', readonly=True)
    net_sale_amount_final_include_tax = fields.Float(string='Net Sale Amount Include Tax', readonly=True)
    gross_amount_final_include_tax = fields.Float(string='Gross Amount Include Tax', readonly=True)
    ...
    def init(self, check_date = '', check_date_gte_or_lte = '', date_gte = datetime.datetime.now().strftime('%Y/%m/%d'), date_lte = datetime.datetime.now().strftime('%Y/%m/%d')):
        ...
        SELECT row_number() OVER(ORDER BY hr_employee_employee_code) AS id , *  FROM(
            (SELECT hr_employee_employee_code,
                ...
                WHEN date between %s and %s THEN sum_cost_price ELSE 0 END) AS sum_cost_price_amount_final,
