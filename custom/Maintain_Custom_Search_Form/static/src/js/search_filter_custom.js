odoo.define('web.search_filters_custom', function (require) {
"use strict";
...
var ExtendedSearchProposition = Widget.extend({
    template: 'SearchView.extended_search.proposition1',
    events: {
...
//        }
    ...
    select_field: function (field) {
        ...
        if(this.attrs.selected !== null && this.attrs.selected !== undefined) {
            ...
            if(this.attrs.selected.string == 'X Studio Business Partner' || this.attrs.selected.string == 'searchkey' || this.attrs.selected.string == 'name'  ||  this.attrs.selected.string == 'description' ){
                if(typeof this.__parentedParent !== 'undefined'){
                if(typeof this.__parentedParent.__parentedParent !== 'undefined'){
                     if(typeof this.__parentedParent.__parentedParent.context !== 'undefined'){
                        if(typeof this.__parentedParent.__parentedParent.context.lang !== 'undefined'){
                            if(this.attrs.selected.string == 'X Studio Business Partner'){
                            ...
                            if(this.attrs.selected.string == 'searchkey'){
                            if(this.__parentedParent.__parentedParent.context.lang ==='en_US'){
                                this.attrs.selected.string = _t('Search Key');
                            }else{
                                this.attrs.selected.string = _t('検索キー');
                            }
                            }

                            if(this.attrs.selected.string == 'name'){
                            if(this.__parentedParent.__parentedParent.context.lang ==='en_US'){
                                this.attrs.selected.string = _t('Name');
                            }else{
                                this.attrs.selected.string = _t('名称');
                            }
                            }

                            if(this.attrs.selected.string == 'description'){
                            if(this.__parentedParent.__parentedParent.context.lang ==='en_US'){
                                this.attrs.selected.string = _t('Description');
                            }else{
                                this.attrs.selected.string = _t('説明');
                            }
                            }
                        }
                     }
            ...
            this.$('.o_searchview_extended_prop_field_lable').html(this.attrs.selected.string);
            $('.o_filters_menu1 .o_menu_item').hide();
            $('.o_filters_menu1 .dropdown-divider').hide();
        }
...
var DateTime = Field.extend({
    ...
    start: function () {
        return Promise.all([
            ...
            this._create_new_widget("datewidget_1", '22:59:59', 'hh:mm:ss'),
        ]).then(() => {
