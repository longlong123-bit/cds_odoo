...
var FilterMenu1 = require('web.FilterMenu_free');
...
var ControlPanelRenderer = Renderer.extend({
     ...
     * @returns {Promise}
     */
    updateState: function (state,ev) {
        this.state = state;
        return this._renderSearch(ev);
    },
     ...
     * @returns {Promise}
     */
    _renderSearch: function (ev) {
        var defs = [];
        var isUpdate = true;
        if(typeof ev !== 'undefined'){
            if (typeof ev.target.dropdownCategory !== 'undefined' && ev.target.dropdownCategory==='filter1'){
                isUpdate = false;
            }
        }
        ...
        if (this.withSearchBar) {
            if(!isUpdate){
                this.$('.o_searchview').html('');
                this.searchBar.facets=[];
                //this.state.facets=[];
                defs = [this._renderSearchBar(isUpdate)];
            }else{
     ...
     * @returns {Promise}
     */
    _renderSearchBar: function (isUpdate) {
...
//            });
            ...
            if (this.state.facets.length>0){
                var lengthF = this.state.facets.length;
                var hasFreeSearch = 0;
                var sF = {};
                var sT = {};

                for (var i = lengthF-1; i>=0 ; i--){
                    var item = this.state.facets[i];
...
//            .toggleClass('o_hidden', !this.displaySearchMenu);
    ...
    _setupMenu: function (menuType) {
        ...
        if (_.contains(['filter1','filter', 'groupBy', 'timeRange'], menuType)) {
