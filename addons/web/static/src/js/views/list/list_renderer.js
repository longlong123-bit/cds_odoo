    ...
    _renderView: function () {
        ...
        var prom = Promise.all(defs).then(function () {
            ...
            var $divTable = $('<div>', {
             ...
             $divTable.prepend('<div class="o_field_x2many_list_row_delete_m"><span></span></div>');

            self.$el.html($divTable);
    ...
    /**
        ...
        _onRowClicked: function (ev) {
            ...
            if (!ev.target.closest('.o_list_record_selector') && !$(ev.target).prop('special_click')) {
                ...
                if (id) {
                    if(ev.delegateTarget.className.includes("dialog_seach_custom")===true){
                        ...
                        $(ev.currentTarget).closest('.modal-dialog').find('.btn-primary').trigger('click');
                        return;
                    }
                    if(ev.delegateTarget.className.includes("dialog_show")===true){
                        ...
                        this.trigger_up('open_record', { id: id, mode: 'edit', target: ev.target, context: {'form_view_initial_mode': 'edit', 'force_detailed_view': 'true'} });
                    }
                    else {
                        this.trigger_up('open_record', { id: id, target: ev.target });
                    }
